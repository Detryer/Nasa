...
class DataBase {
  ...
  private function __construct() {
    $this->mysqli = new mysqli("localhost", "y96759yn_admin", "Vadim2303391", "y96759yn_admin");
  ...
  public function updateFields ($table, $fields, $values, $where = false){

    if(count($fields) == count($values)){
      for($i = 0, $set = ""; $i < count($fields); $i++) {
        $trim = $i < count($fields) - 1 ? ", " : " ";
        $set .= "$fields[$i] = $values[$i]" . $trim;
      }
      $query = "UPDATE `$table` SET `$set`";
      $query .= $where ? "WHERE $where" : "";
      $this->query($query, array());
    } else {
      echo "Количество полей не совпадает с количеством значений!";
    }
  }
